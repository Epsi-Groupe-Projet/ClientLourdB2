	...
	def __init__(self,ptitle,pconnexion,pprecedentResult):
		...
		for element in dicoFenetre[self.mtitle]:
			...
			if element[0] == 'ListeDeroulante':
				...
				var = StringVar()
				self.mdicoResult['listResultEntry'].append(var)
				self.AjoutListeDeroulante(element[1],frameParent,self.mdicoResult['listResultEntry'][self.mcompteurEntry],resultat,element[5],element[6])
				self.mcompteurEntry += 1
			...
			if element[0] == 'ListeDynamiqueButton':
				if element[2] == None:
					...
					frameParent = self.mdicoFrame[element[2]]
				requete = "SELECT " + element[3] + " FROM " + element[7]
				if element[5] != None:
					if element[5] == '#SelectedButton#':
						element[5] = self.mprecedentResult['selectedButton']

						requete_id_parent = "SELECT id_" + element[7] + " FROM " + element[7] + " WHERE libelle_" + element[7] + " = '" + element[5] + "'"
						element[5] = str(executeRequete(self.mconnexion,requete_id_parent)[0][0])

					if element[5] == "Null":
						requete += " WHERE " + element[4] + " IS " + element[5]
					else:
						requete += " WHERE " + element[4] + " = " + element[5]
